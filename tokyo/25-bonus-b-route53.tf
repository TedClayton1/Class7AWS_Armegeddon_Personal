...
locals {
  ...
  bowtiez_zone_name = var.domain_name

  # Explanation: Use either Terraform-managed zone or a pre-existing zone ID (students choose their destiny).
  bowtiez_zone_id = var.manage_route53_in_terraform ? aws_route53_zone.bowtiez_zone01[0].zone_id : var.route53_hosted_zone_id

  # Explanation: This is the app address that will growl at the galaxy (app.bos-growl.com).
  bowtiez_app_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_route53_zone" "bowtiez_zone01" {
  count = var.manage_route53_in_terraform ? 1 : 0

  name = local.bowtiez_zone_name
...
# # Error Resolution: Record already existed
# resource "aws_route53_record" "bowtiez_acm_validation_records01" {
...
# resource "aws_acm_certificate_validation" "bowtiez_acm_validation01_dns_bonus" {
#   count = var.certificate_validation_method == "DNS" ? 1 : 0

#   certificate_arn = aws_acm_certificate.bowtiez_cert.arn

#   validation_record_fqdns = [
#     for r in aws_route53_record.bowtiez_acm_validation_records01 : r.fqdn
#   ]
# }
